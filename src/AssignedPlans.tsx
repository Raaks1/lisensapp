  ...
  return (
    <div className="shadow p-5 mb-4 mt-4 bg-light rounded-3 text-dark">
      <Container fluid>
        ...
        <p className="lead">
          Logged in user's personal license and service information
        </p>
        <AuthenticatedTemplate>
          <>
            <Card
            ...
            >
              ...
              <Card.Body className="cards">
                <div className="p1">
                  <div className="h3">Name:</div> {app.user?.displayName}
                </div>
                <div className="p1">
                  <div className="h3">UserPrincipalName: </div>{" "}
                  {app.user?.userPrincipalName}
                </div>
                <div className="p1">
                  <div key={app.user?.id} className="h3">
                    User id:
                  </div>{" "}
                  {app.user?.id}{" "}
                </div>
 ...
 <p> {plan?.assignedDateTime}</p>
              ...
              {app.lisens?.map((lisens) => (
                <Fragment key={lisens.id}>
                  <Card.Body className="cards">
                    <p>
                      <span className="detail">SkuId:</span> {lisens.skuId} (The
                    ...
                    <p>
                      <span className="detail">Id:</span> {lisens.id}
                    </p>{" "}
                  </Card.Body>
                  <div className="hider">
                    <Card.Header className="headerstwo">
                    ...
                    <>
                      {lisens.servicePlans?.map(
                        ({ servicePlanName, servicePlanId, appliesTo }) => (
                          <p key={servicePlanId}>
                            <span className="plann"> ServicePlanName:</span>{" "}
                            {servicePlanName},
                            <span className="plann"> ServicePlanId:</span>{" "}
                            {servicePlanId} {/* AppliesTo: {appliesTo}*/}{" "}
                          </p>
                        )
                      )}
                    </>
...
const isTheSame =
  domedata1.every((x) =>
    domedata2.map((d) => d.servicePlanId).includes(x.servicePlanId)
  ) &&
  domedata2.every((x) =>
    domedata1.map((d) => d.servicePlanId).includes(x.servicePlanId)
  );
